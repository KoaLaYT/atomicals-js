...
export const getInputUtxoFromTxid = async (
  utxo: UTXO,
  electrumx: ElectrumApiInterface
) => {
...
export const getFundingSelectedUtxo = async (
  address: string,
  ...
  if (!utxos.length) {
    throw new Error(
      `Unable to select funding utxo, check at least 1 utxo contains ${minFundingSatoshis} satoshis`
    );
  }
...
export const getFundingUtxo = async (
  ...
  address: string,
  amount: number,
  suppressDepositAddressInfo = false,
  seconds = 5
) => {
  ...
  if (!suppressDepositAddressInfo) {
    console.log(
      `WAITING UNTIL ${amount / 100000000} BTC RECEIVED AT ${address}`
    );
  }
  console.log(`...`);
  console.log(`...`);
  ...
  while (true) {
    try {
      fundingUtxo = await electrumxApi.waitUntilUTXO(
        address,
        amount,
        seconds ? 5 : seconds,
        false
      ...
      console.log(`electrumxApi.waitUntilUTXO timeout`, err);
    }
  }
  console.log(
